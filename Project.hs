...
bigramFreqHelper (x1:x2:xs) acc | search (x1,x2) acc =  bigramFreqHelper (x2:xs) (inc (x1,x2) acc)
...
generateNextWord[x1, x2] list|(length(filter (generateNextWordHelper [x1,x2]) list) == 0) = error "Sorry, it is not possible to infer from current database"
